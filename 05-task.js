'use strict';
/*
  Задача, добавьте возможность удалять отдельные задачи в списке. Изменения должны сохраняться в localStorage
  */
...
let dataService = {
    ...
    get notSelected() {
        return this.tasks.filter(task => task.selected = false);
    },
...
class TasksListView {
    ...
    drawNotDeleted(){
    let taskElements = [];
        let tasks = dataService.notSelected();
        if (tasks.length == 0) return;
...
showNotCompletedButton.after(deleteButton);
